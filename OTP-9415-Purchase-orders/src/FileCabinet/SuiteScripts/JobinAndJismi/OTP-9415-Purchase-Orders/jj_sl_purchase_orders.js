    ...
    try {
      ...
      if (request.method === 'GET') {
        ...
        for (let i = 0; i < lineCount; i++) {
          ...
          supervisorPOMap[emailAddr].pos.push({ docnum, memo, vendor, amount });
        }

        // Resolve supervisorId for each email
        Object.entries(supervisorPOMap).forEach(([emailAddr, data]) => {
          try {
            // Attempt to find supervisorId from email
            ...
            if (supervisorId) {
              ...
              log.audit({
                title: 'Email Sent',
                details: `Report emailed to ${emailAddr} (ID: ${supervisorId}) with ${poList.length} POs.`
              });
            } else {
              log.error({
                title: 'Supervisor ID Not Found',
                details: `Could not find supervisor ID for email: ${emailAddr}`
              });
            }
          } catch (err) {
            log.error({
